...
import { Appointment, Doctor, StatusAppointment } from "@/lib/types"
...
export function UpdateAppointmentModal({ appointment, open, onOpenChange }: UpdateAppointmentModalProps) {
  ...
  const onSubmit = async (data: Appointment) => {
    ...
    if (!data.prescription || data.prescription.length === 0) {
      delete (data as {prescription: unknown}).prescription
    }
    console.log("data submit",data);
    
    updateAppointment.mutate({data, appointmentID: appointment._id}, {onSuccess: ()=> handleClose()})
  }
  // console.log("new Date() >= new Date(appointment.date)",(new Date() < new Date(appointment?.followUpDate as Date)
  //                       ||(watch("followUpDate") != undefined && new Date() < new Date(watch("followUpDate") as Date))));
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              {...register(`status`, {
                validate: value => {
                  if (value !== "scheduled" && new Date() < new Date(appointment.date)) {
                    return "Date not coming yet.";
                  }

                  const followUpDate = watch("followUpDate") ?? appointment.followUpDate;
                  if (value !== "scheduled" && followUpDate && new Date() < new Date(followUpDate)) {
                    return "This appointment has a follow-up date.";
                  }
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              ...
              <Button
                ...
                variant="outline"
                onClick={addMedicine}
                disabled={fields.length > 0 && (!watch("prescription")[fields.length - 1]?.medicine?.trim() || !watch("prescription")[fields.length - 1]?.dose?.trim()) && fields.length != 0}
              ...
              <Popover>
                ...
                <PopoverContent className="w-auto p-0" align="start">
                  <Calendar
                    ...
                    disabled={(date) => date < new Date() || !doctor.shift.days.includes(date.getDay())}
                    // classNames={{
